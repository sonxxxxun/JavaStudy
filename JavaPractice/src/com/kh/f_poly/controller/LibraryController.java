...
public class LibraryController {
	...
	private Member men = null;   			//null로 명시 초기화
	...
	//초기화 불록
		...
		public void insertMember(Member men) {
				this.men = men;						//전달 받은 men 주소 값을 해당 회원 레퍼런스(mem)에 대입
			}
		
		public Member mylnfo() {
			    return men;     					// 회원 레퍼런스(mem) 주소값 리턴
		 	}
		
		public Book[] selectAll() {
			return this.bList; 						// 도서 목록 레퍼런스(bList) 주소 값 리턴
		}
		
		public Book[] searchBook(String keyword) {
			Book[] result = new Book[5];							// 검색 결과를 담아줄 새로운 Book 객체 배열 생성
